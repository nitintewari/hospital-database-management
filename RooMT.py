...
def room_button():
    ...
    tkinter.messagebox.showinfo("VIT HOSPITAL MANAGEMENT SYSTEM", "ROOM ALLOCATED")
    conn.commit()
    conn.close()
...
def update_button():
    ...
    if len(p) != 0:
        ...
        tkinter.messagebox.showinfo("VIT HOSPITAL MANAGEMENT SYSTEM", "ROOM DETAILS UPDATED")
        conn.commit()
    else:
        tkinter.messagebox.showinfo("VIT HOSPITAL MANAGEMENT SYSTEM", "PATIENT IS NOT ALLOCATED A ROOM")
...
def ROOMD_button():
    ...
    if (len(p)==0):
        tkinter.messagebox.showinfo("VIT HOSPITAL MANAGEMENT SYSTEM","PATIENT NOT ALLOCATED ROOM")
    else:
